import { GroupsToChars } from "@/types";

export const groupsToChars: GroupsToChars = {
    "la_1": [ ["シ", "shi", "k_shi"], ["ツ", "tsu", "k_tsu"], ["ソ", "so", "k_so"], ["ン", "n", "k_n"], ["ノ", "no", "k_no"], ["メ", "me", "k_me"] ],
    "la_2": [ ["ウ", "u", "k_u"], ["ワ", "wa", "k_wa"], ["フ", "fu", "k_fu"], ["ラ", "ra", "k_ra"], ["ヲ", "wo", "k_wo"], ["ヌ", "nu", "k_nu"], ["ス", "su", "k_su"] ],
    "la_3": [ ["れ", "re", "h_re"], ["ね", "ne", "h_ne"], ["わ", "wa", "h_wa"] ],
    "la_4": [ ["ア", "a", "k_a"], ["ヤ", "ya", "k_ya"], ["マ", "ma", "k_ma"] ],
    "la_5": [ ["め", "me", "h_me"], ["ぬ", "nu", "h_nu"] ],
    "la_6": [ ["き", "ki", "h_ki"], ["さ", "sa", "h_sa"], ["ち", "chi", "h_chi"], ["ら", "ra", "h_ra"] ],
    "la_7": [ ["せ", "se", "h_se"], ["サ", "sa", "k_sa"] ],
    "la_8": [ ["う", "u", "h_u"], ["つ", "tsu", "h_tsu"] ],
    "la_9": [ ["に", "ni", "h_ni"], ["こ", "ko", "h_ko"] ],
    "la_10": [ ["り", "ri", "h_ri"], ["リ", "ri", "k_ri"], ["い", "i", "h_i"], ["け", "ke", "h_ke"] ],
    "la_11": [ ["メ", "me", "k_me"], ["ナ", "na", "k_na"], ["テ", "te", "k_te"], ["チ", "chi", "k_chi"], ["モ", "mo", "k_mo"] ],
    "la_12": [ ["コ", "ko", "k_ko"], ["ユ", "yu", "k_yu"] ],
    "la_13": [ ["ク", "ku", "k_ku"], ["ケ", "ke", "k_ke"], ["タ", "ta", "k_ta"] ],
};